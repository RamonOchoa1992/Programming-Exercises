...
export const letter_combinations = str => {
    ...
    if (count !== 0) return "You must enter a string with digits from 2 through 9."
...
export const numToEng = num => {
    ...
    if (numStringLength > 3) return "You must enter a number that has a max length of 3 characters."

    const unitChar = +numString.charAt(numStringLength - 1);
    const tenChar = numStringLength !== 1 ? +numString.charAt(numStringLength - 2) : undefined;
    const hundredChar = (numStringLength !== 1 && numStringLength !== 2) ? +numString.charAt(numStringLength - 3) : undefined;
    ...
    const tenString = (number.tenChar === undefined || number.tenChar === 0)
        ? ""
        : number.tenChar === 1
            ? getTenOneString(number.unitChar) + " "
            : getTenString(number.tenChar) + " ";

    const hundredString = (number.hundredChar === undefined || number.hundredChar === 0)
        ? ""
...
export const join = (arr = undefined) => {
    ...
    if (arr.length === 0) return "You must enter an array with values inside of it."
    ...
    arr.forEach((el, index, arr) => {
        if (index > 0) {
            ...
            for (let i = 1; i <= splitedFirstPosition.length; i++) {
                if (splitedFirstPosition.slice(-i).join("") === splitedSecondPosition.slice(0, i).join("")) {
    ...
    return [response.join(""), arr.length === 1 || arrSharedLetter.length === 0 ? 0 : minimumValue];
}
    ...
    for (let i = 0; i < grid.length; i++) {
        for (let j = 0; j < grid[i].length; j++) {
            ...
            if (grid[i][j] instanceof Array) {
                if (grid[i][j][0] === letter.toUpperCase() || grid[i][j][1] === letter.toUpperCase()) {
                    number = (i + 1).toString() + (j + 1).toString();
                }
            }
        }
...
const numberToLetter = (grid, num) => {
    ...
    const letter = (grid[firstPosition - 1][secondPosition - 1] instanceof Array) ? "I" : grid[firstPosition - 1][secondPosition - 1];
...
export const polybius = str => {
    ...
    if (count !== 0 && count !== splittedStrLength) return "You must enter a string with all the elements are numbers or all elements are letters."
...
export const getObject = obj => {
    ...
    let index = 0;
    const anotherDataObject = dataObject.filter(({ isItAlready }) => !isItAlready);
...
export const nicoCipher = (message, key) => {
    ...
    for (let i = 0; i < response.length; i++) {
        ...
        for (let j = 0; j < positionKey.length; j++) {
            response[i][positionKey[j] - 1] = message[j] !== undefined ? message[j] : " ";
        }
    }
...
export const movingParticles = arr => {
    if (arr.length === 0) return "No particles are in the list, you must enter an array."
...
export const knightsJump = square => {
    ...
    const orderedResponse = [...response].sort((a, b) => a.charAt(1) - b.charAt(1))
...
export const expressFactors = num => {
    ...
    if (num <= 1 || num >= 10000) return "You must enter a value contained between 1 and 10,000."
    ...
    for (const i in primeFac) {
        finalResponse += `${anotherCount === 0 ? '"' : ""}${i}${primeFac[i] > 1 ? "^" + primeFac[i] : ""}`;
        anotherCount++;
...
export const fiscalCode = ({ name, surname, gender, dob }) => {
    ...
    if (countName > 0) return `The name of the data must be cointaned by vowels and consonants.`

    const isVowel = letter => {
        if (letter.toUpperCase().charCodeAt() === 65 || letter.toUpperCase().charCodeAt() === 69 || letter.toUpperCase().charCodeAt() === 73 || letter.toUpperCase().charCodeAt() === 79 || letter.toUpperCase().charCodeAt() === 85) {
            return true
        }
        else {
            return false;
    ...
    const vowelArray = str => {
        const response = str.split("").filter(el => isVowel(el)).map(el => el.toUpperCase()).join("");
        return response;
    }

    const consonantArray = str => {
        const response = str.split("").filter(el => isConsonant(el)).map(el => el.toUpperCase()).join("");
        return response;
    }
    ...
    const letterSurname = (surname.length < 3)
        ? `${consonantArray(surname).charAt(0)}${vowelArray(surname).charAt(0)}X`
        : (consonantArray(surname).length < 3)
            ? `${consonantArray(surname).slice(0, consonantArray(surname).length)}${vowelArray(surname).slice(0, 3 - consonantArray(surname).length)}`
            : consonantArray(surname).slice(0, 3)


    const letterName = (name.length < 3)
        ? `${consonantArray(name).charAt(0)}${vowelArray(name).charAt(0)}X`
        : (consonantArray(name).length < 3)
            ? `${consonantArray(name).slice(0, consonantArray(name).length)}${vowelArray(name).slice(0, 3 - consonantArray(name).length)}`
            : (consonantArray(name).length === 3)
    ...
    const lastTwoNumber = (gender === "M" && dayNumber < 10)
        ? `0${dayNumber}`
        : (gender === "M" && dayNumber >= 10)
            ? dayNumber
            : dayNumber + 40

    const responseCode = letterSurname + letterName + yearDigite + monthLetter + lastTwoNumber
...
const amountAdjacentHash = (initialPos, finalPos, grid) => {
    ...
    const maxInitialPos = initialPos === grid.length - 1 ? grid.length - 1 : initialPos + 1;
    const minFinalPos = finalPos === 0 ? finalPos : finalPos - 1;
    const maxFinalPos = finalPos === grid[0].length - 1 ? grid[0].length - 1 : finalPos + 1;
